...
// Modes: buses (OSETH feed, OSM roadways, navy) and metro (synthesized feed, OSM
// tunnels, red). Usage: node pipeline/build.mjs [--all | lines...] [--tram M1]
// The metro feed (pipeline/metro-feed.mjs, built from OSM route relations because
// no metro GTFS exists) ships NO shapes.txt — for those lines the pipeline uses
// the station sequence as sparse HMM observations and lets Viterbi route between
// stations along the OSM tunnel network.
...
import { buildNameDict, greekTitleCase, latinize } from './lib/greek.mjs';
...
const MODES = [{
  mode: 'bus', label: 'buses (OSETH)', gtfsDir: 'data/gtfs', osmFile: 'data/osm/thessaloniki.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['01X']),
}];
// Thessaloniki has no trolleybuses (the last ones ran in 2011) — the trolleybus
// green of the sibling maps simply never triggers here, route_type is 3 throughout.
if (tramLines.length) MODES.push({
  mode: 'tram', label: 'metro (OSM-derived feed)', gtfsDir: 'data/gtfs-t', osmFile: 'data/osm/thessaloniki-rail.json',
  graphMode: 'tram', color: '#e30613', colorDark: '#7c060e',
  // official Thessaloniki color: Line 1 red (Line 2, the Kalamaria extension, is
  // blue #0070ff — still under construction, so it is not in the map yet)
  lineColors:     { M1: '#e30613', M2: '#0070ff' },
...
async function processMode(cfg) {
  ...
  // the metro feed has no shapes.txt — geometry is reconstructed from stop sequences
  ...
  for (const r of reps) {
    ...
    if (r.pseudo) {
      ...
      // fills the narrow net so this line's tunnel would never be seen at an
      // interchange. perWay keeps the list diverse despite dense station tracks.
      sampled = xy;
  ...
  for (const e of stopAgg.values()) {
    ...
    // metro gets a wide snap net: station coords in a feed can be entrance-based
    // and sit well off the track axis — the disc belongs ON the line
...
// `nolabel` (its stroke stays, the track takes over its numbers).
{
  ...
  // Metro NEVER adopts street numbers: a metro line is one long run, so the
  // adoption union would collect every bus line the tunnel passes under — a label
  // listing lines that do not ride that street. Only street-running rail (trams in
  // the sibling maps) shares corridors with buses.
...
const nameFeatures = mergeRuns(streetFeatures
  ...
  .map((r) => {
    // Second line of the label: the name transliterated the way Greece signs
    // its own streets (ELOT 743, see lib/greek.mjs). A name that is already Latin
...
await (await import('./night.mjs')).nightPass(outDir, /\dN$/, { sort: false });
