// Report of gaps in the GTFS feed's shapes.txt (OSETH) — for manual verification
...
const osm = JSON.parse(readFileSync(join(ROOT, 'data/osm/thessaloniki.json'), 'utf8'));
const CELL = 250;
...
const md = `# Gap report for shapes.txt — Thessaloniki (OSETH) bus GTFS

- **Feed:** ${feedInfo.feed_publisher_name || 'OSETH (data.gov.gr)'}, version \`${feedInfo.feed_version || '?'}\` (valid ${feedInfo.feed_start_date || '?'}–${feedInfo.feed_end_date || '?'}), contact: ${feedInfo.feed_contact_email || '—'}
- **Generated:** ${new Date().toLocaleString('en-GB', { dateStyle: 'short', timeStyle: 'short' })} by the \`thessaloniki-bus-map\` pipeline
